...
/*   Updated: 2025/02/12 18:10:47 by mstefano         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		int new_width, int new_height)
{
	...
	while (y < new_height)
	{
		...
		while (x < new_width)
		{
			...
			src_index = ((y * texture->height / new_height) * texture->width
					+ temp) * 4;
			dst_index = (y * new_width + x) * 4;
			copy_pixel(&scaled->pixels[dst_index], &texture->pixels[src_index]);
			x++;
		}
		y++;
	}
